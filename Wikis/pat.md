...
## Make your player skull play a sound when a player right clicks it with a stick (Using actAsInteractable)
...
function events.skull_render()
  ...
  -- Check if the player is looking at a player head and holding an item
  local check = viewer:getTargetedBlock().id:find("head") and
      viewer:getHeldItem().id ~= "minecraft:air"
  FOXAPI.foxpat.config.actAsInteractable = check -- Set the actAsInteractable variable to the result
  avatar:store("foxpat.actAsInteractable", FOXAPI.foxpat.config.actAsInteractable)
  tick = false -- Set tick to false, make sure this event only runs once every tick
end

function events.skull_pat(entity, state, coord)
  if state == 1 then return end -- Don't play the sound if the state is 1 (State of 1 is when you stop being patted)
  local heldItem = entity:getHeldItem().id

  if heldItem == "minecraft:stick" then
    sounds:playSound("minecraft:block.note_block.bell", coord)
    interaction = true
  end

  return { not interaction, true } -- Always cancel particles, but cancel swinging if the interaction variable is false.
end
```
...
## `ENTITY_PAT()`
This event runs when you get patted or unpatted. The state is the status of the pat event. `0` is for when you're first patted, `1` is when you stop being patted, and `2` is for the duration of you being patted.
...
## `SKULL_PAT()`
This event runs when one of your skulls gets patted or unpatted. The state is the status of the pat event. `0` is for when you're first patted, `1` is when you stop being patted, and `2` is for the duration of you being patted.
